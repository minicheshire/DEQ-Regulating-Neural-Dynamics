...
def parse_args():
    ...
    parser.add_argument('--step_size',
                        help='step size alpha for perturbation',
                        type=int,
                        default=2)
    parser.add_argument('--num_steps',
                        ...
                        type=int,
                        default=10)
    parser.add_argument('--is_mixed',
                        help='whether the adv training is mixed with std training',
                        action='store_true')
    parser.add_argument('--need_adv_pretrain',
                        help='need adv training during pretrain',
                        action='store_true')
...
def main():
    ...
    if args.resume_epoch != "":
        model_state_file = os.path.join(final_output_dir, 'checkpoint_{}.pth.tar'.format(args.resume_epoch))
...
#    print(sum(p.numel() for p in model.module.state_dict().values()))#model.module.state_dict())
#    cao = input()
    ...
    for epoch in range(last_epoch, config.TRAIN.END_EPOCH):
        ...
        train(config, train_loader, model, criterion, optimizer, lr_scheduler, epoch,
              final_output_dir, tb_log_dir, writer_dict, topk=topk, epsilon=args.epsilon/255, step_size=args.step_size/255, num_steps=args.num_steps, is_mixed=args.is_mixed, need_adv_pretrain=args.need_adv_pretrain)
        ...
        if epoch >= 30:
            ...
            perf_indicator_pgd = validate(config, valid_loader, model, criterion, lr_scheduler, epoch,
                                      final_output_dir, tb_log_dir, writer_dict, 
                                      topk=topk, spectral_radius_mode=config.DEQ.SPECTRAL_RADIUS_MODE, epsilon=8/255, step_size=2/255, num_steps=10)
            
            if perf_indicator_pgd > best_perf:
                best_perf = perf_indicator_pgd
                best_model = True
            else:
                best_model = False
        ...
        save_checkpoint({
            ...
            'perf': 0 if epoch < 30 else perf_indicator_pgd,
            'optimizer': optimizer.state_dict(),
            'lr_scheduler': lr_scheduler.state_dict(),
            'train_global_steps': writer_dict['train_global_steps'],
        }, best_model, final_output_dir, filename='checkpoint_{}.pth.tar'.format(epoch+1))
