...
def main():
    ...
    if config.TEST.MODEL_FILE:
        ...
        if args.testModel != "":
#            model_state_file = os.path.join(final_output_dir, 'checkpoint_{}.pth.tar'.format(args.testModel))
            logger.info('=> loading model from {}'.format(args.testModel))
            model.load_state_dict(torch.load(args.testModel)["state_dict"])
        else:
...
#            normalize,
    ...
    validate(config, valid_loader, model, criterion, None, epoch=-1, output_dir=final_output_dir,
             ...
             epsilon=args.epsilon/255, step_size=args.step_size/255, num_steps=args.num_steps)
