...
class MDEQNet(nn.Module):
    ...
    def _forward(self, x, train_step=-1, compute_jac_loss=True, spectral_radius_mode=False, writer=None, isEval=False, **kwargs):
        """
        ...
        if not deq_mode:
            ...
            with torch.no_grad():
                ...
                result = self.f_solver(func, z1, threshold=torch.randint(1,f_thres+1,(1,))[0].item() if not isEval else f_thres, stop_mode=self.stop_mode, name="forward")
                z1 = result['result']
#            new_z1 = z1
