...
from lib.solvers import anderson, broyden, matvec, rmatvec
...
class MDEQNet(nn.Module):
    ...
    def _forward(self, x, train_step=-1, compute_jac_loss=True, spectral_radius_mode=False, writer=None, AttackGrad="unroll", for_attack=False, loss_func=None, **kwargs):
        """
        ...
        x_list = [self.stage0(x_ds) if self.stage0 else x_ds]
        for i in range(1, num_branches):
            bsz, _, H, W = x_list[-1].shape
            x_list.append(torch.zeros(bsz, self.num_channels[i], H//2, W//2).to(x_ds))   # ... and the rest are all zeros
            
        z_list = [torch.zeros_like(elem) for elem in x_list]
        z1 = list2vec(z_list)
#        if bias is None:
#            bias = torch.zeros_like(z1)
        cutoffs = [(elem.size(1), elem.size(2), elem.size(3)) for elem in z_list]
        func = lambda z: list2vec(self.fullstage(vec2list(z, cutoffs), x_list))
        ...
        jac_loss = torch.tensor(0.0).to(x_ds)
        sradius = torch.zeros(bsz, 1).to(x_ds)
        ...
        if not deq_mode:
            ...
            if not for_attack:
                # Here comes our input entropy reduction method
                assert loss_func is not None
                now_x = x.clone().detach()
                for o_iter in range(1):
                    now_func = func #lambda z: list2vec(self.fullstage(vec2list(z, cutoffs), now_x_list))
                    z1_list = []
                    now_Us = now_VTs = None
                    now_nstep = 0
                    now_z1 = list2vec([torch.zeros_like(elem) for elem in x_list])
                    for now_th in range(f_thres//2):
                        with torch.no_grad():
                            result = self.f_solver(now_func, now_z1, threshold=2, stop_mode=self.stop_mode, name="forward", _Us=now_Us, _VTs=now_VTs, _nstep=now_nstep) # Here the T_f=2
                            z1 = result['result']
                            for _ in result['results_list']: z1_list.append(_)
                            now_z1 = result['result']
                            now_nstep = result['nstep']
                            now_Us  = result['Us']
                            now_VTs = result['VTs']
                            now_delta_x  = result['delta_x']
                            now_delta_gx = result['delta_gx']
                            now_gx       = result['gx']
                        
                        for kk in range(10): # And here R=10
                            with torch.enable_grad():
                                now_x.requires_grad_()
                                now_x_ds = self.downsample(now_x)
                                now_x_list = [self.stage0(now_x_ds) if self.stage0 else now_x_ds]
                                for ii in range(1, num_branches):
                                    bsz, _, H, W = now_x_list[-1].shape
                                    now_x_list.append(torch.zeros(bsz, self.num_channels[ii], H//2, W//2).to(now_x_ds))
                                now_z1.requires_grad_()
                                now_loss = loss_func(self.fullstage(vec2list(now_z1, cutoffs), now_x_list)) #list2vec(self.fullstage(vec2list(now_z1, cutoffs), now_x_list)))
                                now_grad    = autograd.grad(now_loss, now_x , retain_graph=True)[0]
                                eta = 2/255 * now_grad.sign()
                                now_x = now_x.detach() - eta # Update the x 
                                now_x = torch.min(torch.max(now_x, x-8/255), x+8/255)
                                now_x = torch.clamp(now_x,0,1)

                        # After the update of x, need to update the `now_func` as it now should receive the updated x as input
                        # Also update the now_u, now_VT, etc. in Broyden's method to `lift the orbit` to regulate neural dynamics
                        now_x_ds = self.downsample(now_x)
                        now_x_list = [self.stage0(now_x_ds) if self.stage0 else now_x_ds]
                        for ii in range(1, num_branches):
                            bsz, _, H, W = now_x_list[-1].shape
                            now_x_list.append(torch.zeros(bsz, self.num_channels[ii], H//2, W//2).to(now_x_ds))
                        now_func = lambda z: list2vec(self.fullstage(vec2list(z, cutoffs), now_x_list))
                        
                        now_part_Us, now_part_VTs = now_Us[:,:,:,:now_nstep-1], now_VTs[:,:now_nstep-1]
                        now_vT = rmatvec(now_part_Us, now_part_VTs, now_delta_x)
                        now_delta_gx = now_delta_gx + (now_func(now_z1)-now_z1) - now_gx
                        now_u  = (now_delta_x - matvec(now_part_Us, now_part_VTs, now_delta_gx)) / torch.einsum('bij, bij -> b', now_vT, now_delta_gx)[:,None,None]
                        now_vT[now_vT != now_vT] = 0
                        now_u[now_u != now_u] = 0
                        now_VTs[:,now_nstep-1] = now_vT
                        now_Us[:,:,:,now_nstep-1] = now_u

            else:
                with torch.no_grad():
                    result = self.f_solver(func, z1, threshold=f_thres, stop_mode=self.stop_mode, name="forward")
                    z1 = result['result']
                    z1_list = result['results_list']

            if not self.training:
                with torch.enable_grad():
                    if "unroll-0" in AttackGrad:
                        new_z1 = func(z1_list[0].requires_grad_())
                    elif "unroll-1" in AttackGrad:
                        new_z1 = func(z1_list[1].requires_grad_())
                    elif "unroll-2" in AttackGrad:
                        new_z1 = func(z1_list[2].requires_grad_())
                    elif "unroll-3" in AttackGrad:
                        new_z1 = func(z1_list[3].requires_grad_())
                    elif "unroll-4" in AttackGrad:
                        new_z1 = func(z1_list[4].requires_grad_())
                    elif "unroll-5" in AttackGrad:
                        new_z1 = func(z1_list[5].requires_grad_())
                    elif "unroll-6" in AttackGrad:
                        new_z1 = func(z1_list[6].requires_grad_())
                    elif "unroll-7" in AttackGrad:
                        new_z1 = func(z1_list[7].requires_grad_())
                    else:
                    ...
                    if AttackGrad == "real":
                        ...
                        def backward_hook(grad):
                            result = self.b_solver(lambda y: autograd.grad(new_z1_cp, z1_cp, y, retain_graph=True)[0] + grad, torch.zeros_like(grad),threshold=b_thres, stop_mode=self.stop_mode, name="backward")
                            self.ret_z1_grad = result['result']
                            return result['result']
                        new_z1.register_hook(backward_hook)
                    elif "unroll" in AttackGrad:
                        if ("unroll-" in AttackGrad) and (not "es-0" in AttackGrad):
                            for _ in range(int(AttackGrad[10])):
                                if "F" in AttackGrad:
                                    new_z1 = func(new_z1)
                                else:
                                    new_z1 = (1-0.5) * new_z1 + 0.5 * func(new_z1)
                        else:
                            for _ in range(9):
                                new_z1 = (1-0.5) * new_z1 + 0.5 * func(new_z1)
                    else:
                        z1_cp = z1.clone().detach().requires_grad_()
                        new_z1_cp = (1-0.5) * z1_cp + 0.5 * func(z1_cp)
                        def backward_hook(grad):
                            grad_hat = grad
                            for _ in range(9-1):
                                grad_hat = grad + autograd.grad(new_z1_cp, z1_cp, grad_hat, retain_graph=True)[0]
#                            grad_hat = autograd.grad(new_z1_cp, z1_cp, grad_hat, retain_graph=True)[0]
                            return 0.5*grad_hat
                        new_z1.register_hook(backward_hook)
            ...
            if self.training:
                ...
                if AttackGrad == "real":
                    ...
                    new_z1_cp = func(z1_cp)
                    def backward_hook(grad):
                        result = self.b_solver(lambda y: autograd.grad(new_z1_cp, z1_cp, y, retain_graph=True)[0] + grad, torch.zeros_like(grad),threshold=30 if "b30" in attack else b_thres, stop_mode=self.stop_mode, name="backward")
                        self.ret_z1_grad = result['result']
                        return result['result']
                    new_z1.register_hook(backward_hook)
                elif AttackGrad == "unroll":
                    for _ in range(9):
                        new_z1 = (1-0.5) * new_z1 + 0.5 * func(new_z1)
                else:
                    z1_cp = z1.clone().detach().requires_grad_()
        ...
        if AttackGrad != "Funroll-es-0":
            y_list_list = [vec2list(now_z1, cutoffs) for now_z1 in z1_list]
        else:
            y_list_list = []
            with torch.enable_grad():
                for z1_id in range(1,len(z1_list)):
                    y_list_list.append(vec2list(func(z1_list[z1_id].requires_grad_()), cutoffs))

        y_list_list.append(y_list)
        return y_list, jac_loss.view(1,-1), y_list_list
