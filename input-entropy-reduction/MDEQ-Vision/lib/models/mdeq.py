...
sys.path.append("../")
from lib.layer_utils import conv3x3, list2vec
...
class MDEQClsNet(MDEQNet):
    ...
    def get_func(self, x, **kwargs):
        return self._get_func(x, **kwargs)

    def forward(self, x, train_step=0, AttackGrad="unroll", for_attack=False, **kwargs):
        loss_func = lambda y_list: -(nn.Softmax(dim=-1).cuda()(self.predict(y_list))*nn.LogSoftmax(dim=-1).cuda()(self.predict(y_list))).sum()/y_list[0].shape[0] # for exp: remove the negative sign

        y_list, jac_loss, y_list_list = self._forward(x=x, train_step=train_step, AttackGrad=AttackGrad, for_attack=for_attack, loss_func=loss_func, **kwargs)

        if "es-0" in AttackGrad:
            now_predict = self.predict(y_list_list[0])
            for _ in y_list_list[1:]:
                now_predict = now_predict + self.predict(_)

            now_predict = now_predict / len(y_list_list)
            del y_list_list
            del y_list
            torch.cuda.empty_cache()
            y_list_list = None
            y_list = None
            return now_predict, None, None
        else:
            now_predict = self.predict(y_list)
            return now_predict, list2vec(y_list), y_list_list
...
class MDEQSegNet(MDEQNet):
    ...
    def forward(self, x, train_step=0, isUnrollGrad=False, **kwargs):
        loss_func = lambda y_list: -(nn.Softmax(dim=-1).cuda()(self.predict(y_list))*nn.LogSoftmax(dim=-1).cuda()(self.predict(y_list))).sum()
        y, jac_loss, sradius = self._forward(x=x, train_step=train_step, isUnrollGrad=isUnrollGrad, loss_func=loss_func, **kwargs)
