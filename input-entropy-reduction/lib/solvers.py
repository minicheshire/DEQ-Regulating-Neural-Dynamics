...
def broyden(f, x0, threshold, eps=1e-3, stop_mode="rel", ls=False, name="unknown", _Us=None, _VTs=None, _nstep=0):
    ...
    while nstep < threshold:
        ...
        results_list.append(x_est.clone().detach())
...
def anderson(f, x0, m=6, lam=1e-4, threshold=50, eps=1e-3, stop_mode='rel', beta=1.0, **kwargs):
    ...
    for k in range(2, threshold):
        ...
        trace_dict['abs'].append(abs_diff)
        trace_dict['rel'].append(rel_diff)
        results_list.append(X[:,k%m].view_as(x0).clone().detach())
